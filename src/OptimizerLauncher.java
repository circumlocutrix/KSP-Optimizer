...
public class OptimizerLauncher {
	public static final double deltaV = 2500;
	public static final double payloadM = 41.791;
	public static final double relativeG = 1.2;
	public static final boolean atmosphere = true;
	
	
	public static final double mutationPercent = 0.95;
	public static ArrayList<Part> partList;
	public static final int partSwaps = 5;
...
//		partList.add(new Engine("LV-T30 Liquid Fuel Engine",1.25,215,370,320));
		...
		partList.add(new Engine("Rockomax 48-7S",.1,30,350,300));
...
//		partList.add(new Engine("Toroidal Aerospike Rocket",1.5,175,390,388));
		...
		operators.add(new EvolutionaryOperator<HashMap<Part,Integer>>(){
			...
			public List<HashMap<Part,Integer>> apply(List<HashMap<Part,Integer>> selectedCandidates, Random rng) {
				List<HashMap<Part,Integer>> newCandidates = new ArrayList<HashMap<Part,Integer>>();
				for (int i =0 ; i+1<selectedCandidates.size();i+=2){
		...
		EvolutionEngine<HashMap<Part,Integer>> engine = new GenerationalEvolutionEngine<HashMap<Part,Integer>>(new AbstractCandidateFactory<HashMap<Part,Integer>>(){
			...
			public double getFitness(HashMap<Part,Integer> candidate,
				...
				double massXenonFuel = 0;
				double xenonThrust = 0;
				double xenonImpDen = 0;
				...
				for (Part p : candidate.keySet()){
					...
					if (p instanceof Engine){
						if (p.ft == Fuel.XENON){
							xenonThrust+=candidate.get(p)*((Engine)p).thrust;
							if (!atmosphere)
								xenonImpDen+=candidate.get(p)*((Engine)p).thrust/((Engine)p).impulse;
							else
								xenonImpDen+=candidate.get(p)*((Engine)p).thrust/((Engine)p).atmoImpulse;
						}
						else{
						...
						if (p.ft == Fuel.XENON)
							massXenonFuel += candidate.get(p)*((Tank)p).fuelMass;
						else
				...
				if (massFuel != 0 && impDen != 0){
					massEmpty = massFull-massFuel;
					impulse = thrust/impDen;
					dV=9.82*impulse*Math.log(massFull/massEmpty);
					if (dV == 0) thrust = 0;
					twr = thrust/(massFull*relativeG*9.82);
				}
				else{
					twr = 1;
					thrust = 0;
				...
				double xenonImpulse = 0;
				double xDV = 0 ;
				double xtwr = 0;
				if (massXenonFuel != 0 && xenonImpDen != 0){

					xenonImpulse = xenonThrust / xenonImpDen;
					xDV = 9.82*xenonImpulse*Math.log(massEmpty/(massEmpty-massXenonFuel));
					if (xDV == 0) xenonThrust = 0;
					xtwr = xenonThrust/(massEmpty*relativeG*9.82);
				...
				double tCo = 0;
				if ((thrust == 0) && (xenonThrust == 0)) return 0;
				if (twr >= 1 && xtwr >= 1) tCo = 1;
				double dvCo = 0;
				if (dV > deltaV) dvCo = deltaV;


				return tCo*dvCo/(massFull+.000001*numParts);//((Math.pow(massFull,2))+.001*(Math.pow(numParts, 1)));
		...
		engine.addEvolutionObserver(new EvolutionObserver<HashMap<Part,Integer>>()
			...
			public void populationUpdate(PopulationData<? extends HashMap<Part,Integer>> data)
			{
				if (data.getGenerationNumber()%100==0){
					...
					System.out.print(" Thrust: "+Math.max(xenonThrust,thrust)+"kN, Full Mass: "+massFull+" Fuel Mass: "+Math.max(massFuel,massXenonFuel)+"  Empty Mass: "+Math.min(massEmpty,massEmpty-massXenonFuel)+", delta V: "+dV+", impulse: "+Math.max(xenonImpulse,impulse)+" # Parts: "+numParts);
		...
		final HashMap<Part,Integer> result1 = engine.evolve(100, 1, new Stagnation(50000,true));//new ElapsedTime(1000*30));//new Stagnation(3000,true));
